...
# 🔒 forwarded message হ্যান্ডলার + safe permission check
    ...
    try:
        if current_state in ["waiting_for_source_forward", "waiting_for_destiny_forward"]:
            try:
                ...
                can_edit = getattr(chat_member, "can_be_edited", False)
                if not (can_post or can_edit):
                    return await message.reply_text(
                        f"⚠️ Bot needs to be an admin in {chat.title} (ID: <code>{chat.id}</code>) with 'Post Messages' permission. Please promote me.",
                        parse_mode=ParseMode.HTML)
